...
- **Domain specs** for logic live under **`/core/<Entity>/logic/__tests__/`** (see **`.cursor/skills/project-structure/SKILL.md`**). Example import: `@core/Game/logic/createGame` from `createGame.test.ts` in that folder. Follow the **repo-wide no-barrel** rule and **camelCase** filenames for multi-word modules.
...
- **`.cursor/skills/tdd/SKILL.md`** — Red / Green / Refactor and where to place new specs.
- **`.cursor/rules/create-react-component.mdc`** — RTL, `data-testid`, and component test conventions for `/app`.
